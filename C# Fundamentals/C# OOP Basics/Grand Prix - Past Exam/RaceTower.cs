...
public class RaceTower
{
    ...
    public string CompleteLaps(List<string> commandArgs)
    {
        ...
        for (int lap = 0; lap < numberOfLaps; lap++)
        {
            ...
            for (int index = 0; index < orderedDrivers.Count - 1; index++)
            {
                ...
                {
                    index++;
                    builder.AppendLine(string.Format(OutputMessages.OvertakeMessage, overtakingDriver.Name, targetDriver.Name, this.track.CurrentLap));
                }
                else
                {
    ...
    private bool TryOverTake(Driver overtakingDriver, Driver targetDriver)
    {
        ...
        bool crash = (aggressiveDriver && this.track.Weather == Weather.Foggy) || (enduranceDriver && this.track.Weather == Weather.Rainy);
